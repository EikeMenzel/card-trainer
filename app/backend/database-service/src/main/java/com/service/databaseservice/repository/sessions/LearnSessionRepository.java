...
public interface LearnSessionRepository extends JpaRepository<LearnSession, Long> {
    ...
    @Query("SELECT ls FROM LearnSession ls WHERE ls.status.type != 'FINISHED' AND ls.createdAt < :timestamp")
    List<LearnSession> findByStatusNotAndCreatedAtBefore(Timestamp timestamp);
}
