...
public interface CardRepository extends JpaRepository<Card, Long> {
    ...
    @Query("SELECT c FROM Card c JOIN RepetitionModel r ON c.id = r.card.id WHERE c.deck.id = :deckId AND r.nextLearnTimestamp < NOW() ORDER BY r.nextLearnTimestamp ASC LIMIT 1")
